...
addShift = function (shift, groupid) { 
  let shiftid = Shifts.insert({'groupid': groupid,start:shift.start,end:shift.end,capacity: shift.cap,swaps:[],users:[]});
...
addSwap = function (date, swapin, swapout, shiftid) {
  let shift = Shifts.findOne({_id: shiftid});
  if (indexUser(swapout, shift.users) >= 0 && indexUser(swapin, shift.users) < 0) {
    shift.users.push({'swapin':swapin,'swapout':swapout,'date':date});
  }
  Shifts.update({_id: shiftid}, {$set:{swaps: shift.users}});
}

notifySub = function (groupid, subout, outid, dateout) {
  let shift = Shifts.findOne({_id: outid});
  Notifications.insert({'groupid': groupid, 'sender': subout, 'shiftid': outid, 'date': dateout, 'startin': shift.start, 'endin': shift.end, 'type': 'sub', 'seen':[]});
}
...
notifySwap = function (groupid, swapout, outid, dateout, inid, datein) {
  ...
  if (notes.length == 0) {
    let id = Notifications.insert({'groupid': groupid, 'sender': swapout, 'shiftid': outid, 'date': dateout, 'startin': shiftin.start, 'endin':shiftin.end, 
                                   'startout': shiftout.start, 'endout': shiftout.end, 'type': 'swap', 'swapid': inid, 'swapdate': datein, seen: []});
...
filterRequests = function (notifArray) {
  ...
  for (i = 0; i < notifArray.length; i ++) {
    if (notifArray[i].sender == curUser) {
...
checkSeen = function (notifArray) {
  ...
  for (i = 0; i < copy.length; i++) {
    ...
    if (copy[i].sender == curUser && copy[i].acceptID == undefined) {
      console.log(copy[i].acceptID);
      record.push(i);
    } 
    // check if user is in the shift involved in the swap request
    else if (swapid) {
      let shift = Shifts.findOne({_id: swapid});
      if (indexUser(curUser,shift.users) < 0) {
        record.push(i);
      } 
    }
    for (j = 0; j < seenArray.length; j++) {
...
isSwappedOut = function (userid, swaps) {
  for (i = 0; i < swaps.length; i++) {
    if (swaps[i].swapout == userid)
      return true;
  }
  return false;
}

isSwappedIn = function (userid, swaps) {
  for (i = 0; i < swaps.length; i++) {
    if (swaps[i].swapin == userid)
      return true;
  }
  return false;
}
